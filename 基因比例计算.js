...
// ancestry.filter(people => (1924 - people.born) < 25 )
// ancestry.filter(people => people.sex === 'm')

function keyBy(ancestry, property) {
  let map = {};
...
let byName = keyBy(ancestry, 'name');

// 计算 name 为 Pauwels van Haverbeke
function geneWithPVH(name) {
  // byName(name).father
  // byName(name).mother
  let people = byName[name];
  ...
  return (geneWithPVH(people.father) + geneWithPVH(people.mother)) / 2;
}
// let list = nameMap(ancestry);
...
//   if (Object.hasOwnProperty.call(byName, key)) {
//     geneMap[key] = geneWithPVH(key)
//   }
// }
...
function countAncestors(person, test) {
  function combine(current, fromMother, fromFather) {
    let thisOneCounts = current != person && test(person);
